...
rsync -avz -e "ssh -i $SSH_KEY" --exclude=node_modules $BACKEND_DIR ubuntu@$AWS_SERVER_IP:~/tmp/
rsync -avz -e "ssh -i $SSH_KEY" --exclude=node_modules $FRONTEND_DIR ubuntu@$AWS_SERVER_IP:~/tmp/
rsync -avz -e "ssh -i $SSH_KEY" $AI_DIR ubuntu@$GCP_SERVER_IP:~/tmp/
...
echo "[1-1] 백엔드 빌드 시작..."
cd ~/release/backend
SENTRY_AUTH_TOKEN="${SENTRY_AUTH_TOKEN}" ./gradlew clean build -x test || {
...
echo "VITE_API_URL=${VITE_API_URL}" >> ~/release/frontend/.env
...
ssh -i $SSH_KEY ubuntu@$GCP_SERVER_IP << EOF
sudo cp -r ~/tmp/ai-server ~/release/
sudo chown -R ubuntu:ubuntu ~/release
echo "✅ release 디렉토리로 복사 완료"

echo "[1-5] AI 서버 준비 및 패키지 설치..."
cd ~/release/ai-server/fastapi_project
if [ ! -d "venv" ]; then
